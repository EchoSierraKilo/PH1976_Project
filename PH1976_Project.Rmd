...
4. Determine models and weights for each sub feature
- For each of the 7, a number of models were trained using the train_df.*_train data. In each instance, the highest performing model (comparatively) was selected as the chosen model for that subfeature (i.e. KNN performed best on baseline, NN performed best on TQWT, etc.)
- Weights were scored from 0 to 1 based on the predictive accuracy of the model (i.e. if the best model for baseline was KNN and performed at 0.79, weight_baseline was set to 0.79

5. Determine overall accuracy of the ensemble
- A weighting function of Y = sum(w_i*d_i) was used. This was chosen based on the paper provided by the project description.
...
- Do the same thing as on the train_df_test data, but this time on the test_df (and with no feedback to establish weights).
...
Additionally, the ensemble training set (both **train_df_train and train_df_test) are broken into the following sub feature categories:  
...
df_results = data.frame(ID = test_data$id,'PredictedResult' = weighted_predictions_test)
unique_rows = !duplicated(df_results$ID)
df_results[unique_rows,]
```